...
let notification_reducer = function (state = [], action) {
    switch (action.type) {
        case 'SHOW': {
            const notification = {text: action.text, id: action.id}
            return [...state, notification];
        }
        case 'HIDE': return state.filter(e => e.id !== action.id);
        default: return state
    }
};

// Creating the store
const store = createStore(notification_reducer, applyMiddleware(logger, thunk));
